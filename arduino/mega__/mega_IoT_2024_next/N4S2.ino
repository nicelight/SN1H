...
// обработка нажатий клавиш N4.S2 ( у входа в мамину комнату)
...
void handleN4S2() {
  /*
    документация:
    //  if (N4_S2_but.isPress()) Serial.println("Press");         // первое срабатывание. если выключен свет, включим и установим флаг о включении
    //  if (N4_S2_but.isSingle()) Serial.println("Single\n");       // одиночный клик. если флаг о включенном свете возведен, ничего не делаем, иначе убираем флаг и меняем состояние света
    //  if (N4_S2_but.isDouble()) Serial.println("Double\n");       // двойной клик. доп освещение
    //  if (N4_S2_but.isTriple()) Serial.println("Triple\n");       // тройной клик. меняем состояние светильников на 1, 2, 1+2.
    //  if (N4_S2_but.isHolded()) Serial.println("Holded\n");      0
  ...
  */
  ...
  if (N4_S2_but.isSingle())
  {
    if (N4_spots.rightNowOn) { // если мгновенно включен свет
      ...
      update_N4_Lamps();
      Serial.println("/nN4_S2_but Single");
    }
  }//N4_spots.isSingle

  // двойной клик. on\off museum + ermitage.
  if (N4_S2_but.isDouble()) {
    N4_spots.rightNowOn = 0; // убираем флаг мгновенного нажатия
    if (N4_museums.state) {
      N4_museums.state = 0;
  ...
  if (N4_S2_but.isHolded()) {
    if (N4_spots.rightNowOn ) {
      ...
      //
      Serial.print("\n\n\t\tN4_S2_but  NIGHT MODE ON\n\n");// TODO отправка режима ночь !!!
    }
  }//holded

  if (N4_S2_but.hasClicks())
  {
    N4_spots.rightNowOn = 0; // убираем флаг мгновенного нажатия
    Serial.print("N4_S2_but multi Clicks: ");
    Serial.println(N4_S2_but.getClicks());
...
Зона N4 управляется обработчиком handleN4S2() и вспомогательными функциями в N4S2.ino.
Кнопка N4_S2_but обрабатывает все типы нажатий, а структура N4_spots хранит режим (lamp1, lamp2, state, rightNowOn, mode).
Ниже логика события за событием.

**Основной цикл**

 - Каждый проход вызывает N4_S2_but.tick() и EE_N4_spots.tick() (N4S2.ino (lines 23-24)),
...
после чего включаются лампы N4_SP1/N4_SP2 (N4S2.ino (lines 27-34), update_N4_Lamps() в N4S2.ino (lines 140-150)).
...
оставляя свет в текущем состоянии — защита от мгновенного двойного реагирования (N4S2.ino (lines 41-44)).
Иначе одиночный клик выключает общий свет (state = 0) и отправляет команды на пины и
Modbus (N4S2.ino (lines 44-47) + update_N4_Lamps()).

**Двойной клик (Double)**

- Сбрасывает rightNowOn и переключает подсветку «музей/эрмитаж» (структура N4_museums): 
если горела — гасит обе линии, если была выключена — включает обе (N4S2.ino (lines 51-63)).
- После обновления состояния вызывается update_N4_museums() (N4S2.ino (lines 152-162)),
...
Перебор идёт через поле mode, при каждой смене соответствующим образом выставляются lamp1/lamp2 (N4S2.ino (lines 65-87)).
- После выбора сцены update_N4_Lamps() вносит изменения в физические выходы и Modbus (N4S2.ino (line 88)),
а EE_N4_spots.update() и EE_N4_museums.update() сохраняют новую комбинацию в EEPROM (N4S2.ino (lines 89-91)).

**Долгое удержание (Holded)**

- Если флаг rightNowOn ещё активен (то есть свет только что включили), удержание трактуется как «режим присутствия»: включает все связанные группы (spots, museums, kitchen, tracks) и обновляет их по очереди с небольшими задержками (N4S2.ino (lines 96-108)).
Если флаг неактивен, удержание считается «ночным режимом»: выключает те же группы и также вызывает update_* последовательно (N4S2.ino (lines 110-126)). Вывод для кухни закомментирован — выглядит как временное решение, пока не реализован update_N4_kitchen().

**Прочее**

hasClicks() сбрасывает rightNowOn и печатает количество кликов, но не меняет выходы (N4S2.ino (lines 129-135)).
...
состояние ламп (N4S2.ino (lines 170-190)).
...
В итоге N4_S2 обслуживает: мгновенное включение при входе, обычное выключение,
