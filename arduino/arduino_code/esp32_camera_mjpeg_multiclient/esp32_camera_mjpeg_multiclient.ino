...
IPAddress staticIP(192, 168, 10, 183); // важно правильно указать третье число - подсеть, смотри пояснения выше
...
void mjpegCB(void* pvParameters) {
  ...
  server.on("/jpg", HTTP_GET, handleJPG); // кадр без вспышки
  server.on("/jpgf", HTTP_GET, handleJPGF); // кадр со вспышкой
...
void handleNotFound()
{
  ...
  message += server.args();
  message += "\nget shot: /jpg";
  message += "\nget shot with flash: /jpgf";
  message += "\nSwitch on Light: /flashon";
  message += "\n\t Light off: /flashoff";
  message += "\n\nOTA update firmware: ip/update";
  server.send(200, "text / plain", message);
}

// ==== Handle toogle FLASH On ============================================
void handleFlashOn()
{
  digitalWrite(4, 1);
  String message = "\n\n\n\tFlash ON";
